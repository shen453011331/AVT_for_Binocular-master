...
//初始化投影仪
bool Projector::Projector_Init(std::string ini_filename)
{
	...
	//Read ini
	ReadIni(ini_filename.c_str());
	//验证是否有编码
	if (PatSeqLUT_data.size() == 0)
	{
		Log += "Error:No Pattern data aviliable.\n";
		return false;
	}
...
void Projector::ReadIni(const char* file_path)
{
